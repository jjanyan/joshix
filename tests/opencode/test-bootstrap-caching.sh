...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$JOSHIX_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$JOSHIX_SKILLS_DIR/using-joshix/SKILL.md" "$TEST_HOME/using-joshix.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$JOSHIX_PLUGIN_FILE" missing
}
