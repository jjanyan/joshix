#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the joshix plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/joshix.js"
...
skill_count=$(find "$JOSHIX_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $JOSHIX_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-joshix skill exists (critical for bootstrap)
echo "Test 3: Checking using-joshix skill (required for bootstrap)..."
if [ -f "$JOSHIX_SKILLS_DIR/using-joshix/SKILL.md" ]; then
    echo "  [PASS] using-joshix skill exists"
else
    echo "  [FAIL] using-joshix skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$JOSHIX_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/joshix/' "$JOSHIX_PLUGIN_FILE"; then
