...
# 1. Create in joshix location (lowest priority)
mkdir -p "$JOSHIX_SKILLS_DIR/priority-test"
cat > "$JOSHIX_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: joshix version of priority test skill
---
# Priority Test Skill (joshix Version)

This is the JOSHIX version of the priority test skill.

PRIORITY_MARKER_JOSHIX_VERSION
EOF
...
if [ -f "$JOSHIX_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] joshix version exists"
else
    echo "  [FAIL] joshix version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled joshix priority
echo ""
echo "Test 2: Documenting personal vs joshix priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled joshix version instead of personal version"

# Test 3: Document project vs bundled joshix priority
echo ""
echo "Test 3: Documenting project vs personal/joshix priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled joshix version instead of project version"

# Test 4: Test a non-colliding bundled joshix skill is still available
echo ""
echo "Test 4: Testing non-colliding joshix skill remains available..."

mkdir -p "$JOSHIX_SKILLS_DIR/joshix-only-test"
cat > "$JOSHIX_SKILLS_DIR/joshix-only-test/SKILL.md" <<'EOF'
---
name: joshix-only-test
description: joshix-only priority test skill
---
# joshix Only Test Skill

PRIORITY_MARKER_JOSHIX_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"joshix-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_JOSHIX_ONLY_VERSION" "Non-colliding joshix skill is still registered"
