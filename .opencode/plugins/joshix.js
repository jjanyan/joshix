/**
 * joshix plugin for OpenCode.ai
 *
 * Injects joshix bootstrap context via system prompt transform.
...
export const JoshixPlugin = async ({ client, directory }) => {
  const homeDir = os.homedir();
  const joshixSkillsDir = path.resolve(__dirname, '../../skills');
  ...
  const getBootstrapContent = () => {
    ...
    // Try to load using-joshix skill
    const skillPath = path.join(joshixSkillsDir, 'using-joshix', 'SKILL.md');
...
**IMPORTANT: The using-joshix skill content is included below. It is ALREADY LOADED - you are currently following it. Do NOT use the skill tool to load "using-joshix" again - that would be redundant.**
...
${toolMapping}
  ...
  return {
    // Inject skills path into live config so OpenCode discovers joshix skills
    ...
    config: async (config) => {
      ...
      if (!config.skills.paths.includes(joshixSkillsDir)) {
        config.skills.paths.push(joshixSkillsDir);
      }
    },
